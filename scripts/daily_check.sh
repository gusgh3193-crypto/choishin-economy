...
# daily_check.sh
#
# 매일 사이트 점검을 위해 아래 세 스크립트를 순서대로 실행하는 통합 스크립트다.
...
#
# 이 스크립트는 기존 세 스크립트의 내용을 절대 수정하지 않고, 그대로 호출만 한다.
...
echo "=========================================="
echo "[1/3] 글 규칙 검증을 실행합니다: validate_posts.sh"
echo "=========================================="
...
echo ""
echo "=========================================="
echo "[2/3] sitemap.xml / robots.txt 생성을 실행합니다: generate_sitemap.sh"
echo "=========================================="
bash "${SITEMAP_SCRIPT}"
sitemap_status=$?
...
echo ""
echo "=========================================="
echo "[3/3] JSON-LD 생성을 실행합니다: generate_jsonld.sh"
echo "=========================================="
bash "${JSONLD_SCRIPT}"
jsonld_status=$?

if [ ${validate_status} -ne 0 ] || [ ${sitemap_status} -ne 0 ] || [ ${jsonld_status} -ne 0 ]; then
  overall_status=1
fi
